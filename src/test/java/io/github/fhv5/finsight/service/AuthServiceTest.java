...
class AuthServiceTest {
    ...
    void rotateTokens_ShouldReturnNewTokens_WhenAccessTokenExpired() {
        ...
        Claims claims = Jwts.claims().id(jti).build();
        ExpiredJwtException expiredException = new ExpiredJwtException(null, claims, "Token is expired");

        when(jwtService.parseClaims("accessToken")).thenThrow(expiredException);
    ...
    void rotateTokens_ShouldThrowException_WhenAccessTokenNotExpired() {
        ...
        when(jwtService.parseClaims("validAccessToken")).thenReturn(claims);

        assertThrows(IllegalArgumentException.class, () -> authService.rotateTokens(authHeader, refreshToken));
    }
}
